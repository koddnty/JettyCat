...
// URL 百分号解码（query string 中 '/' -> %2F 需还原）
...
// ---------------------------------------------------------------------------
// HTTP 接口: POST /files/upload/sts
// 只给用户"写单个对象"的权限（对象 key = <目标snow_id>/<文件hash>）
// ---------------------------------------------------------------------------
m_sylar::Task<void> coUploadSts(m_sylar::http::HttpSession::ptr session) {
    ...
    if (jwt.empty() || JWT::verifyJWT(jwt) != JWT::State::SUCCESS) {
        M_SYLAR_LOG_WARN(j_logger) << "coUploadSts, unauthorized: invalid or missing JWT token";
        sendResp(session, 403, "FORBIDDEN: Invalid or missing JWT token");
        co_await session->co_sendResp();
        co_return;
    }

    // 协议层职责：取参数（query string 中的 '/' 会被编码为 %2F，需先还原）
    const std::string target_kind = req->getParam("kind");
    const std::string target_id_str = urlDecode(req->getParam("target"));
    const std::string file_hash = urlDecode(req->getParam("hash"));
    const std::string duration_str = req->getParam("duration");

    // 业务逻辑交给 service（越权校验 + 单对象写凭证）
    const auto result = co_await getFileService().getUploadSts(
        JWT::parserPayload(jwt), target_kind, target_id_str, file_hash, duration_str);

    if (!result.isOk()) {
        M_SYLAR_LOG_WARN(j_logger) << "coUploadSts, business failed: " << result.getMsg();
    }
...
void registeUrl(m_sylar::http::HttpServer::ptr server) {
    ...
    server->POST("/files/upload/sts", coUploadSts);
}
